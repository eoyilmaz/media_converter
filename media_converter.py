...
class Manager(object):
    ...
    converter_data = [
        ...
        },
        {
            ...
            "-preset veryslow " "{extra_options} " '"{output_file_full_path}"',
        },
        {
            "name": "audio_to_aac",
        ...
        },
        {
            ...
            "command": 'ffmpeg -i "{input_file_full_path}" '
            # '-c:a aac -b:a 192k '
            "{extra_options} " '"{output_file_full_path}"',
        },
        {
            "name": "denoise_normal",
            "file_types": VIDEO_FORMATS + IMAGE_FORMATS,
        ...
        },
        {
            ...
            "-vf format=yuv422p9le "
            # '-preset veryslow '
            "-threads 16 " "{extra_options} " '"{output_file_full_path}"',
        },
        {
            "name": "prores422sq",
            "file_types": VIDEO_FORMATS + IMAGE_FORMATS,
    ...
    def run_per_file(self, f):
        ...
        if is_image_sequence:
            ...
            self.command = self.command.replace(
                "ffmpeg", "ffmpeg -start_number %s" % start_number
            )
        ...
        output_file_full_path = os.path.join(self.target_path, output_file_name)
                ...
                if not self.auto_rename:
                    ...
                    while os.path.exists(output_file_full_path):
                        ...
                        output_file_full_path = os.path.join(
                            self.target_path, new_output_filename
                        )
                        i += 1
...
def main():
    ...
    parser.add_argument(
        "-o",
        "--output",
        help="The output folder, if skipped it will be same" "with the input folder",
    )
    parser.add_argument("-x", "--extra-options", help="Extra options", default="")
    parser.add_argument(
        "-a",
        "--auto-rename",
    ...
    parser.add_argument(
        "-c", "--command-info", help="Print command info.", action="store_true"
    )
    ...
    if not converter_name:
        parser.print_usage()
        print(
            "media_converter: error: the following arguments are required: -t/--template"
        )
        sys.exit(-1)
